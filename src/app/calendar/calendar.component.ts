...
export class Calendar implements OnInit, OnDestroy, ControlValueAccessor {
  ...
  @ViewChild('secondInputField', {static: false}) secondInput: ElementRef;
  ...
  mask: HTMLDivElement | any;

  maskClickListener: Function | any;

  overlay: HTMLDivElement | any;
  ...
  yearPickerValues() {
    ...
    let base: any = this.currentYear - (this.currentYear % 10);
    for (let i = 0; i < 10; i++) {
    ...
    for (let i = 0; i < monthRows; i++) {
      ...
      if (i == 0) {
        for (let j = prevMonthDaysLength - firstDay + 1; j <= prevMonthDaysLength; j++) {
          ...
          const day = {
            day: j,
            dateTime: new Date(prev.year, prev.month, j),
            month: prev.month,
            year: prev.year,
            otherMonth: true,
            isIntoRange: false,
        ...
        for (let j = 0; j < remainingDaysLength; j++) {
          week.push({
            day: dayNo,
            dateTime: new Date(year, month, dayNo),
            month: month,
            year: year,
            isIntoRange: false,
        ...
        for (let j = 0; j < 7; j++) {
          if (dayNo > daysLength) {
            ...
            week.push({
              ...
              dateTime: new Date(next.year, next.month, dayNo - daysLength),
              month: next.month,
              year: next.year,
              isIntoRange: false,
              otherMonth: true,
  ...
  leaveFromDate(): void {
    // 1 faqat berun az range toza shavad
    // 2 agar 2 select nashuda boshad hamash toza shavad
    if (!this.value) {
      return;
    } else if (!(this.value[1] instanceof Date)) {
      this.rangeDates.forEach(day => day.isIntoRange = false);
    } else {
      for (let day of this.rangeDates) {
  ...
  hoverRange(event, dateMeta) {
    console.log(1)
    if (this.disabled || !dateMeta.selectable || !this.value) {
      return;
    }
    for (let day of this.rangeDates) {
      if (this.isRangeSelection()) {
        if (this.value[0]) {
          const endDate = new Date(dateMeta.year, dateMeta.month, dateMeta.day);
          day.isIntoRange = this.isDateBetween(this.value[0], endDate, day)
        } else if (this.value[1]) {
          const startDate = new Date(dateMeta.year, dateMeta.month, dateMeta.day);
          day.isIntoRange = this.isDateBetween(startDate, this.value[1], day)
        }
      } else {
        const endDate = new Date(dateMeta.year, dateMeta.month, dateMeta.day);
        day.isIntoRange = this.isDateBetween(this.value[0], endDate, day)
  ...
  updateInputfield() {
    ...
    if (this.secondInput && this.secondInput.nativeElement) {
      this.secondInput.nativeElement.value = this.endDate;
    }
  }
  ...
  isSelected(dateMeta): boolean | any {
    if (!this.dateMeta.length) {
      this.dateMeta.push(dateMeta)
    }
    if (this.value) {
      if (this.isSingleSelection()) {
        ...
        if (this.value[1]) {
          if (this.value[0]) {
            dateMeta.isIntoRange = this.isDateBetween(this.value[0], this.value[1], dateMeta)
          }
  ...
  isActive(dateMeta): boolean | any {
    if (this.value) {
      if (this.isRangeSelection()) {
        if (this.value[1]) {
          return this.isDateBetween(this.value[0], this.value[1], dateMeta);
        } else return this.isDateEquals(this.value[0], dateMeta);
      }
    } else {
      return false;
  ...
  isDateBetween(start, end, dateMeta): boolean {
  ...
  switchViewButtonDisabled() {
    return this.numberOfMonths > 2 || this.disabled;
  }
  ...
  trapFocus(event) {
    let focusableElements: any = DomHandler.getFocusableElements(this.contentViewChild.nativeElement);
  ...
  repeat(event: Event, interval: number | any, type: number, direction: number) {
    let i = interval || 500;
  ...
  parseValueFromString(text: string): Date | Date[] | any {
  ...
  destroyMask() {
    ...
    let hasBlockerMasks: boolean | any;
